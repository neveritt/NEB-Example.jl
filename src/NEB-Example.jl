...
nebxoptions = IdOptions(iterations = 20, autodiff=true, estimate_initial=false)
# two stage settings
fir_m  = n
orders = [n, m, nᵤ, nᵣ, N]
firmodel = FIR(fir_m*ones(Int,1,nᵣ),ones(Int,1,nᵣ), 1, nᵣ)
options = IdOptions(iterations = 5, autodiff=true, estimate_initial=false)
...
@sync @parallel for i = 1:MC
  y,u,r,y0,u2 = _create_data(N,m,nᵤ,nᵣ,s0,nsru,nsry,Θ₀,Θ₂)
  ...
  NEBXtrace, z = NetworkEmpiricalBayes.nebx(y[1:1,:],u,r,y[2:2,:]- r[2:2,:], orders, Ts, nebxoptions)
